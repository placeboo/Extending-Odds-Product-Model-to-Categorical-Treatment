...
max.likelihood.v3 = function(y, z, va, vb, alpha.start, beta.start, max.step=1000, thres=10^-5){
      ...
      while(diff > thres & step < max.step){
            ...
            for(alpha_index in c(1: num_rr)){
                  ...
                  print(paste("coverage=", opti$convergence))
                  alpha[,alpha_index] = opti$par # update
                  diff_alpha[alpha_index] = Diff(opti$par, alphai)
            }

            #print(paste("alpha_diff=", diff_alpha))
            opt2 = stats::optim(beta, #method = "BFGS",
                                neg.log.likelihood.beta, control = list(maxit = max.step))

            #print(opt2$convergence)
            diff2 = Diff(opt2$par, beta)
            #print(paste("beta_diff =", diff2))
            beta = opt2$par
      ...
      for (alpha_index in c(1: num_rr)){
            alphai = alpha[, alpha_index]
            hessian.mat = try(stats::optimHess(alphai, neg.log.likelihood.alpha, control = list(ndeps = c(1e-6, 1e-6))), silent  = TRUE)
            if ("try-error" %in% class(hessian.mat)) { # there is no hession (non finite)
                  next
            }

            # hessian.mat = stats::optimHess(alphai, neg.log.likelihood.alpha)
            # hessian.mat = hessian(neg.log.likelihood.alpha, alphai)
            #print(hessian.mat)
            #grad.mat = grad(neg.log.likelihood.alpha, alphai)
            #print(grad.mat)
            sd.mat[,alpha_index] = sqrt(diag(solve(hessian.mat)))
      }
