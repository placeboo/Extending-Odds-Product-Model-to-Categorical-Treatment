...
max.likelihood.v3 = function(y, z, va, vb, alpha.start, beta.start, max.step=1000, thres=10^-5){
      ...
      neg.log.likelihood = function(pars){
            ...
            alpha = matrix(pars[1: (pa * num_rr)], nrow = pa, ncol = num_rr)
            beta = pars[(pa * num_rr + 1) : (pa * num_rr + pb)]
      ...
      # }
      #
      hessian.mat = try(stats::optimHess(c(alpha, beta), neg.log.likelihood), silent  = TRUE)
      if ("try-error" %in% class(hessian.mat)){
            sd.mat = matrix(NA,  ncol = num_rr, nrow = pa)
      } else {
            sd.mat = matrix(sqrt(diag(solve(hessian.mat)))[1:(pa*num_rr)], ncol = num_rr, nrow = pa)
      }
      opt = list(alpha, beta, step < max.step,  neg.log.likelihood(c(alpha, beta)), sd.mat)
